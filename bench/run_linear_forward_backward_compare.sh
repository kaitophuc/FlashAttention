...
python3 bench/bench_linear_forward_backward_pytorch.py \
  --shapes "$SHAPES_CSV" \
  --output "$TORCH_CSV" \
  --warmup "$WARMUP" \
  --iters "$ITERS"
...
python3 bench/compare_linear_forward_backward_bench.py \
  --native "$NATIVE_CSV" \
  --torch "$TORCH_CSV" \
  --output "$COMPARE_CSV"

echo "[4/4] Plotting comparison charts"
if ! python3 bench/plot_linear_forward_backward_compare.py --compare "$COMPARE_CSV" --outdir "$RESULT_DIR"; then
