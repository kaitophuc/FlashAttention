...
__global__ void fill_linear_data(
    float* x,
    float* w,
    float* b,
    size_t x_numel,
...
void benchmark_linear_forward(benchmark::State& state, const LinearCase* cfg) {
    if (!cuda_available()) {
    ...
    const size_t max_numel = std::max(x_numel, std::max(w_numel, b_numel));
    const int threads = 256;
    const int blocks = static_cast<int>((max_numel + threads - 1) / threads);
    fill_linear_data<<<blocks, threads, 0, stream.s>>>(
    ...
    const double flops = 2.0 * static_cast<double>(cfg->m) * static_cast<double>(cfg->k) * static_cast<double>(cfg->n);
...
int main(int argc, char** argv) {
    ...
    const std::string shapes_path = env_str("FA_LINEAR_SHAPES", "bench/linear_shapes.csv");
    const int iters = env_int("FA_LINEAR_ITERS", 200);
    ...
    benchmark::AddCustomContext("warmup", std::to_string(env_int("FA_LINEAR_WARMUP", 50)));

    for (const LinearCase& c : cases) {
        const std::string bench_name =
            "LinearForward/name=" + c.name +
            "/m=" + std::to_string(c.m) +
            "/k=" + std::to_string(c.k) +
            "/n=" + std::to_string(c.n) +
            "/bias=" + std::to_string(c.bias ? 1 : 0);
        benchmark::RegisterBenchmark(bench_name.c_str(), benchmark_linear_forward, &c)
            ->UseManualTime()
            ->Iterations(iters);
    }
