...
def parse_args():
    ...
    p.add_argument("--native", default="bench/results/native_linear_forward_backward.csv")
    p.add_argument("--torch", default="bench/results/torch_linear_forward_backward.csv")
    p.add_argument("--output", default="bench/results/linear_forward_backward_compare.csv")
    return p.parse_args()
...
def parse_case_from_name(name: str):
    # Example:
    # LinearForwardBackward/name=mlp_s/m=32/k=256/n=256/bias=0/mode=full
    parts = name.split("/")
    out = {}
    for p in parts:
        if "=" in p:
...
def load_native(path: str) -> Dict[Tuple[str, int, int, int, int, str], dict]:
    ...
    for row in reader:
        name = row.get("name", "")
        if not name.startswith("LinearForwardBackward/"):
            continue
        ...
        flops = forward_backward_flops(m, n, k, bias == 1)
