...
def parse_args():
    ...
    p.add_argument("--output", default="bench/results/linear_compare.csv")
    return p.parse_args()
...
def parse_case_from_name(name: str):
    # Example:
    # LinearForward/name=mlp_s/m=32/k=256/n=256/bias=0
    parts = name.split("/")
    out = {}
    for p in parts:
        if "=" in p:
...
def load_native(path: str) -> Dict[Tuple[str, int, int, int, int], dict]:
    out = {}
    ...
    for row in reader:
        name = row.get("name", "")
        if not name.startswith("LinearForward/"):
            continue
        ...
        key = (case_name, m, k, n, bias)
        out[key] = {
            "native_latency_ms": latency_ms,
            "native_tflops": (2.0 * m * k * n) / (latency_ms * 1e-3) / 1e12,
        }
    return out


def load_torch(path: str) -> Dict[Tuple[str, int, int, int, int], dict]:
    out = {}
    with open(path, "r", newline="") as f:
...
def main():
    ...
    rows.sort(key=lambda r: (r["name"], int(r["bias"])))
    ...
    if rows:
        print("name,bias,native_ms,torch_ms,speedup(torch/native)")
        for row in rows:
            print(
                f"{row['name']},{row['bias']},{row['native_latency_ms']},"
