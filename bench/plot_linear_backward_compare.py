...
def parse_args():
    ...
    p.add_argument("--compare", default="bench/results/linear_backward_compare.csv")
    p.add_argument("--outdir", default="bench/results")
    p.add_argument("--prefix", default="linear_backward_compare")
    return p.parse_args()


def find_latest_compare_csv(outdir: str) -> str:
    candidates = sorted(glob.glob(os.path.join(outdir, "linear_backward_compare_*.csv")))
    if not candidates:
        return ""
    return candidates[-1]
...
def backward_bytes_touched(m: int, n: int, k: int, has_bias: bool) -> float:
    # Approx runtime bytes (fp32):
    # dX mm: read dY + read W + write dX
    # dW mm: read dY + read X + write dW
    # db reduce: read dY + write db (if bias)
    dy_bytes = m * n * 4
    x_bytes = m * k * 4
    w_bytes = n * k * 4
    dx_bytes = m * k * 4
    ...
    total = (dy_bytes + w_bytes + dx_bytes) + (dy_bytes + x_bytes + dw_bytes)
    if has_bias:
...
def main():
    ...
    if not os.path.exists(compare_path):
        ...
        if latest:
            ...
            print(
                ...
                "hint: run bench/run_linear_backward_compare.sh first",
                file=sys.stderr,
            )
            return 1
    ...
    prefix = args.prefix
    if prefix == "linear_backward_compare":
        stem = os.path.splitext(os.path.basename(compare_path))[0]
        if stem.startswith("linear_backward_compare_"):
            prefix = stem
    ...
    bytes_per_case = [
        backward_bytes_touched(r["m"], r["n"], r["k"], r["bias"] == 1)
        for r in rows
    ]
    ...
    ax.set_title("Linear Backward Latency by Case (FP32)")
    ...
    ax.set_title("Linear Backward Throughput by Case (FP32)")
    ...
    ax.set_title("Linear Backward Effective Runtime Bandwidth by Case (FP32)")
    ax.set_ylabel("GB/s")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=30, ha="right")
    ax.grid(axis="y", linestyle="--", alpha=0.3)
    ...
    ax2.set_title("Linear Backward Latency Comparison (Grouped Bars)")
