...
__global__ void fill_linear_backward_data(
    float* x,
    float* w,
    float* b,
    float* dy,
...
void benchmark_linear_backward(benchmark::State& state, const LinearCase* cfg) {
    if (!cuda_available()) {
    ...
    fill_linear_backward_data<<<blocks, threads, 0, stream.s>>>(
    ...
    const double mm_flops = 4.0 * static_cast<double>(cfg->m) * static_cast<double>(cfg->n) * static_cast<double>(cfg->k);
    const double bias_flops = cfg->bias ? static_cast<double>(cfg->m) * static_cast<double>(cfg->n) : 0.0;
    const double flops = mm_flops + bias_flops;
...
int main(int argc, char** argv) {
    ...
    const std::string shapes_path = env_str("FA_LINEAR_BWD_SHAPES", "bench/linear_shapes.csv");
    const int iters = env_int("FA_LINEAR_BWD_ITERS", 200);
    ...
    benchmark::AddCustomContext("warmup", std::to_string(env_int("FA_LINEAR_BWD_WARMUP", 50)));

    for (const LinearCase& c : cases) {
        const std::string bench_name =
            "LinearBackward/name=" + c.name +
            "/m=" + std::to_string(c.m) +
            "/k=" + std::to_string(c.k) +
            "/n=" + std::to_string(c.n) +
            "/bias=" + std::to_string(c.bias ? 1 : 0) +
            "/mode=full";
        benchmark::RegisterBenchmark(bench_name.c_str(), benchmark_linear_backward, &c)
            ->UseManualTime()
            ->Iterations(iters);
    }
